...
const queenThreat = function(board){
  ...
  for(let i = 1; i <= rowsBelow; i++){
    ...
    if(stepRight <= 7 && board[stepDown][stepRight] === 1){
      return true;
    }
    if(stepLeft >= 0 && board[stepDown][stepLeft] === 1){
      return true;
    }
    stepDown += 1;
    stepRight += 1;
  ...
  for(let j = 1; j <= rowsAbove; j++){
    ...
    if(stepRight <= 7 && board[stepUp][stepRight] === 1){
      return true;
    }
    if(stepLeft >= 0 && board[stepUp][stepLeft] === 1){
      return true;
    }
    stepUp -= 1;
    stepRight += 1;
